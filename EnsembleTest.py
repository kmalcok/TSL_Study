...
def load_data_random_sample(DIR, sample_size_per_class=20):
    data = []
...
def testModel(models):
    test_data = load_data_random_sample(DATASET_PATH, sample_size_per_class=20)
    test_images = [i[0] for i in test_data]
    test_labels = np.array([i[1] for i in test_data])

    alexnet_accuracy = evaluate_model(alexnet_model, test_images, test_labels, IMG_SIZE_ALEXNET, "AlexNet", grayscale=True)
    ...
    alexnet_images = np.array([preprocess_image(img, IMG_SIZE_ALEXNET, grayscale=True) for img in test_images]).reshape(-1,
                                                                                                                        IMG_SIZE_ALEXNET,
                                                                                                                        IMG_SIZE_ALEXNET,
                                                                                                                        1) / 255.0
    ...
    test_images = [alexnet_images, inceptionv3_images, mobilenet_images]
...
models = [alexnet_model, inceptionv3_model, mobilenet_model]
testModel(models)
