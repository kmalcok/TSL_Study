...
# Model Test Etme
def testModel(models, accuracies):
    ...
    for index in range(len(test_labels)):
        ...
        # En yüksek doğruluğa sahip modelin tahmini alınır
        best_model_index = np.argmax(accuracies)
        best_prediction = np.argmax(predictions[best_model_index], axis=1)[0]

        label_index = np.where(test_labels[index] == 1)[0][0]
        predicted = best_prediction == label_index
...
model_accuracies = [accuracy_alexnet, accuracy_vgg16, accuracy_mobilenet]

# Ensemble modelini test et
models = [alexnet_model, vgg16_model, mobilenet_model]
testModel(models, model_accuracies)
