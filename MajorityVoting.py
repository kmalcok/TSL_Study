...
def ensemble_predict(models, test_images):
    predictions = np.zeros((test_images[0].shape[0], 26))
    for model, images in zip(models, test_images):
...
def testModel(models):
    ...
    for index, _ in enumerate(test_labels):
        prediction = ensemble_predict(models, [images[index:index + 1] for images in test_images])
