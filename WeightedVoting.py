...
def ensemble_predict(models, test_images, weights):
    predictions = np.zeros((test_images[0].shape[0], 26))
    for model, images, weight in zip(models, test_images, weights):
        predictions += weight * model.predict(images)
    return np.argmax(predictions, axis=1)


# Model Test Etme
def testModel(models, weights):
    ...
    for index, _ in enumerate(test_labels):
        prediction = ensemble_predict(models, [images[index:index + 1] for images in test_images], weights)
        label_index = np.where(test_labels[index] == 1)[0][0]
        predicted = prediction == label_index
...
accuracy_mobilenet = 0.98
accuracy_alexnet = 0.99
accuracy_vgg16 = 1.0

# Toplam doğruluk oranı
total_accuracy = accuracy_mobilenet + accuracy_alexnet + accuracy_vgg16

# Her modelin ağırlığı
weight_mobilenet =   (accuracy_mobilenet / total_accuracy)
weight_alexnet =   (accuracy_alexnet / total_accuracy)
weight_vgg16 =   (accuracy_vgg16 / total_accuracy)

# Model ağırlıkları
model_weights = [weight_alexnet, weight_vgg16, weight_mobilenet]

# Ensemble modelini test et
models = [alexnet_model, vgg16_model, mobilenet_model]
testModel(models, model_weights)
